...
public class SelectionTaskMeasure : MonoBehaviour
{
    ...
    public void StartOneTask()
    {
        ...
        objectTStartingPos = taskUI.transform.position + taskUI.transform.forward * 2f + taskUI.transform.up * 3f;
    ...
    public void EndOneTask()
    {
        ...
        MovableObject mObject = objectT.GetComponent<MovableObject>();
        mObject.detach(grapplingHook);
        grapplingHook.attachedMovables.Remove(objectT.GetComponent<MovableObject>());
        Destroy(objectT);
        Destroy(targerT);
